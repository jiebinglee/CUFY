...
//using ChinaUnicom.Fuyang.Common.File;
...
namespace ChinaUnicom.Fuyang.Framework.Logging
{
    ...
    public class RollingFileAppender : FileAppender
    {
            ...
            lock (this)
            {
                ...
                else
                {
                    if (LogLog.IsErrorEnabled)
                    {
                        ...
                        {
                            LogLog.Error(null,"RollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" + fileName + "] already exists.");
                        }
                    }
                }
        ...
        {
            string path = Configurator.Instance.GetConfigByName("log-output-path");
            ...
            string newFileName = Path.Combine(path, "CHIEF" + DateTime.Now.ToString("yyyyMMdd").ToString() + ".log");
        ...
        private void DetermineCurSizeRollBackups()
        {
            ...
            LogLog.Debug(null,"RollingFileAppender: curSizeRollBackups starts at [" + m_curSizeRollBackups + "]");
        }
        ...
        private ArrayList GetExistingFiles(string baseFilePath)
        {
            ...
            LogLog.Debug(null,"RollingFileAppender: Searched for existing files in [" + directory + "]");
            return alFiles;
        }
        ...
        private void RollOverIfDateBoundaryCrossing()
        {
            if (m_staticLogFileName && m_rollDate)
            {
                if (FileExists(m_baseFileName))
                {
                    ...
                    LogLog.Debug(null,"RollingFileAppender: [" + last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "] vs. [" + m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "]");
                    ...
                    if (!(last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo).Equals(m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo))))
                    {
                        ...
                        LogLog.Debug(null,"RollingFileAppender: Initial roll over to [" + m_scheduledFilename + "]");
                        RollOverTime(false);
                        LogLog.Debug(null,"RollingFileAppender: curSizeRollBackups after rollOver at [" + m_curSizeRollBackups + "]");
                    }
                }
            }
        }
        ...
        protected void ExistingInit()
        {
            ...
            if (AppendToFile == false)
            {
                ...
                if (fileExists)
                {
                    ...
                    {
                        LogLog.Debug(null,"RollingFileAppender: Output file [" + fileName + "] already exists. MaxSizeRollBackups is 0; cannot roll. Overwriting existing file.");
                    }
                    else
                    {
                        LogLog.Debug(null,"RollingFileAppender: Output file [" + fileName + "] already exists. Not appending to file. Rolling existing file out of the way.");
        ...
        private void InitializeFromOneFile(string baseFile, string curFileName)
        {
            ...
            if (m_rollDate && !m_staticLogFileName)
            {
                ...
                {
                    LogLog.Debug(null,"RollingFileAppender: Ignoring file [" + curFileName + "] because it is from a different date period");
                    return;
                }
            }
            ...
            try
            {
                ...
                if (SystemInfo.TryParse(curFileName.Substring(index + 1), out backup))
                {
                    if (backup > m_curSizeRollBackups)
                    {
                        ...
                        LogLog.Debug(null,"RollingFileAppender: File name [" + curFileName + "] moves current count to [" + m_curSizeRollBackups + "]");
                    }
                }
            }
            catch (FormatException)
            {
                ...
                LogLog.Debug(null,"RollingFileAppender: Encountered a backup file not ending in .x [" + curFileName + "]");
            }
        }
        ...
        private RollPoint ComputeCheckPeriod(string datePattern)
        {
            ...
            for (int i = (int)RollPoint.TopOfMinute; i <= (int)RollPoint.TopOfMonth; i++)
            {
                ...
                LogLog.Debug(null,"RollingFileAppender: Type = [" + i + "], r0 = [" + r0 + "], r1 = [" + r1 + "]");
        ...
        protected void RollFile(string fromFile, string toFile)
        {
            if (FileExists(fromFile))
            {
                ...
                try
                {
                    LogLog.Debug(null,"RollingFileAppender: Moving [" + fromFile + "] -> [" + toFile + "]");
                    using (SecurityContext.Impersonate(this))
                    {
            ...
            {
                LogLog.Warn(null,"RollingFileAppender: Cannot RollFile [" + fromFile + "] -> [" + toFile + "]. Source does not exist");
            }
        }
        ...
        protected void DeleteFile(string fileName)
        {
            if (FileExists(fileName))
            {
                ...
                {
                    LogLog.Debug(null,"RollingFileAppender: Exception while moving file to be deleted [" + fileName + "] -> [" + tempFileName + "]", moveEx);
                }
                ...
                try
                {
                    ...
                    LogLog.Debug(null,"RollingFileAppender: Deleted file [" + fileName + "]");
                }
                catch (Exception deleteEx)
                {
                    ...
                    else
                    {
                        ...
                        LogLog.Debug(null,"RollingFileAppender: Exception while deleting temp file [" + fileToDelete + "]", deleteEx);
                    }
                }
            }
        }
        ...
        protected void RollOverSize()
        {
            ...
            LogLog.Debug(null,"RollingFileAppender: rolling over count [" + ((CountingQuietTextWriter)QuietWriter).Count + "]");
            LogLog.Debug(null,"RollingFileAppender: maxSizeRollBackups [" + m_maxSizeRollBackups + "]");
            LogLog.Debug(null,"RollingFileAppender: curSizeRollBackups [" + m_curSizeRollBackups + "]");
            LogLog.Debug(null,"RollingFileAppender: countDirection [" + m_countDirection + "]");
